...
| 正稿续写 | `按全书设定续写`、`从这里接入正文继续`、`从这段开始继续全书` | 把片段作为正式正文入口，加载相关动态状态，写章节并更新 delta |
...
- 先定位目标章节；如果无法判断，会只问一个问题：这段接入第几章，还是作为下一章开头？
- 根据片段里出现的人物、关系、线索和世界规则，筛选读取 `story-state.md`、`thread-ledger.md`、`characters.md`、`world.md` 的相关条目。
- 把用户片段当作即时文风锚点，把上一章当作全书文风锚点。
- 写正式章节，生成 `mode: "standard"` delta，并更新项目状态。

如果用户要求“从这里一直写到完结”，项目状态完整时会先提醒 full-auto 质量风险，然后进入生产锁定；状态不完整或方向不清楚时，会先进入断书接诊。
...
| 远距离章节 | 一句话摘要 | 长期记忆 |
| 当前大纲行 | 完整读取 | 明确本章目标 |
| 动态状态 | 标准写作、正稿续写、生产锁定、断书续写时加载相关条目 | 人物、关系、伏笔、世界规则。片段续写默认不加载完整动态状态，除非作者要求接入正稿 |
