...
3. **Reader Promise** — from `novel/reader-promise.md`, legacy `novel/market-brief.md`, or `context-brief.md`; only target reader, core promise, hook, and style contract
...
5. **Dynamic State** — prefer `novel/generated/story-state.md`; use legacy `novel/story-state.md` only if generated state is missing. Include only the current pressure, relevant character rows, active open threads, continuity locks, and relationship shifts for this chapter.
